...
fn build_regex_matcher(
	...
	// Delegates the leaf matcher build to pi-grep-core while preserving the
	// upstream line-terminator ladder: for a non-multiline pattern, first try
	// `line_terminator: Some(b'\n')` so `.` and the line scanner respect line
	// boundaries (and whole-file `matcher.is_match` fast paths never match across
	// a `\n`), falling back to no terminator only if that build fails. The core's
	// other `MatcherSpec` knobs stay at their defaults, and `build_matcher` over a
	// one-element slice == `RegexMatcherBuilder::build(pattern)`, so matches and
	// error strings are byte-identical to the prior single-pattern builder.
